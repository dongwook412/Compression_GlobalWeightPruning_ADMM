...
def load_weights_func(model, model_name):
    try:
        model.load_weights('/home/ubuntu/weight_pruning_admm/Result/weights/Resnet56/cifar10/train_step/weights')
...
def cifar_resnet110(block_type='preactivated', shortcut_type='B', l2_reg=1e-4, load_weights=False):
    model = Resnet(input_shape=(32, 32, 3), n_classes=10, l2_reg=l2_reg, group_sizes=(18, 18, 18),
                   features=(16, 32, 64),
                   strides=(1, 2, 2), first_conv={"filters": 16, "kernel_size": 3, "strides": 1},
                   shortcut_type=shortcut_type,
                   block_type=block_type, preact_shortcuts=False)
...
def cifar_resnet1001(shortcut_type='B', load_weights=False, l2_reg=1e-4):
    model = Resnet(input_shape=(32, 32, 3), n_classes=10, l2_reg=l2_reg, group_sizes=(111, 111, 111),
                   features=(64, 128, 256),
                   strides=(1, 2, 2), first_conv={"filters": 16, "kernel_size": 3, "strides": 1},
                   shortcut_type=shortcut_type,
                   block_type='bootleneck', preact_shortcuts=True)
