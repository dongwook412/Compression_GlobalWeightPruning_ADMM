...
def train_step(writer, model, image_data, target, X_val, Y_val, optimizer, k_step, epochs, total_steps, global_steps, retraining_total_steps, rho, p_lambda, Z_dict=None, U_dict=None, is_admm =True, dict_nzidx=None, k=0, epoch=0): # W 학습
        """
        Weight 최적화
        ...
        with tf.GradientTape() as tape:
            ...
            if is_admm:  # admm일때만 Z,U를 통해 최적화, retrain일때는 origin_loss만 사용
                for i, layer in enumerate(layers):
                    if 'conv2d' in layer.name or 'dense' in layer.name:
                        weight_loss = weight_loss + tf.nn.l2_loss(layer.weights[0])  # f(W)에서의 W에 대한 l2-loss. 전체layer로하면 batchnormal도껴서안됨
            ...
            # 7번식 loss 완료
            total_loss = origin_loss+ p_lambda*weight_loss + rho*admm_loss # 각 parameter를 곱해줌(lambda, rho)
            ...
            if not is_admm:  # retraining step에서만 쓰임. gradient들을 projection시킴(기존 코드에서 apply_prune_on_grads와 같음)
                ...
                for i, trainable_weight in enumerate(
                        model.trainable_weights):  # model.trainable_weights는 trainable한 layer들의 이름과 weight 정보를 가지고 있음
                    name = trainable_weight.name.split('/')[0]  # conv2d/kernel:0 형식으로 나와서 split
                    if 'conv2d' in name or 'dense' in name:
                        gradients[i] = tf.multiply(tf.cast(tf.constant(dict_nzidx[name][0]), tf.float32), gradients[
                            i])  # dict_nzidx는 0인 부분이 False인 형태이므로 이를 0.0으로 변경(True는 1.0)하고 gradients와 곱해줌
            ...
            if global_steps%400==0 or global_steps==total_steps: # 출력 정도 조절
                #val_pred_result = model.predict(X_val)
                ...
                if is_admm:
                    # admm step
                    tf.print("=> [k-step : %d/%d, epoch : %d/%d]  STEP %4d/%4d   admm_loss: %8.6f  weight_loss: %4.2f  origin_loss: %4.2f  total_loss: %4.2f  val_acc: %4.2f" % (
                    ...
                    # retraining step
                    tf.print("=> STEP %4d/%4d  total_loss: %4.2f, val_acc: %4.2f" % (
                                 global_steps, retraining_total_steps, total_loss, val_acc))
...
def apply_prune(model, W_dict, all_percent=10):
    """
